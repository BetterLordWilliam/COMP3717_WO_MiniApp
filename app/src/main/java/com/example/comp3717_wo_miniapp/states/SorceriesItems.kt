...
class SorceriesItems (override val repo : EldenRingRepo) : EldenRingItemGroup<Sorcery> {
    override val items = mutableStateListOf<ItemState<Sorcery>>()
    override var page = mutableIntStateOf(0)
    override var searchTerms = mutableStateOf("")

    override suspend fun getItems() {
        val response = repo.getSorceries(searchTerms.value, page.intValue)
